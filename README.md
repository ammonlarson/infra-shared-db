...
`dnf-automatic` is set to `reboot = never` on purpose: a surprise reboot would drop an in-progress operator tunnel or `terraform apply`. Most security fixes (libraries, the SSM agent, OpenSSL, …) take effect without a reboot. The kernel/glibc updates that *do* need a reboot are picked up when the bastion is recreated. Recreation is deliberate: `aws_instance.bastion` sets `lifecycle { ignore_changes = [ami] }`, so a new AL2023 AMI published into the SSM parameter no longer replaces the bastion on an unrelated `terraform apply` (which used to inject a surprise destroy into the "additions only" project-add gate and churn the tunnel outputs). To roll onto a newer image, recreate it explicitly with `terraform apply -replace=aws_instance.bastion` (or an operator can reboot it during a quiet window). Because `user_data` only runs on first boot, the instance is set to `user_data_replace_on_change = true`, so applying this change (and any later edit to the script) recreates the bastion cleanly rather than leaving a stale host. A recreate briefly interrupts tunnels — run it like any other bastion change, when no operator session is mid-flight.
